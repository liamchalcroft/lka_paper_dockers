...
class Loss(nn.Module):
    def __init__(self, focal, shape=False):
        ...
        self.shape_loss = (
            ShapeDistLoss(include_background=False, softmax=True, to_onehot_y=True)
            if shape
            else None
        )
    ...
    def forward(self, y_pred, y_true):
        ...
        if self.shape_loss:
            loss += self.shape_loss(y_pred, y_true)
        return loss
...
class LossBraTS(nn.Module):
    def __init__(self, focal, shape=False):
        ...
        self.ce = (
            FocalLoss(gamma=2.0, to_onehot_y=False) if focal else nn.BCEWithLogitsLoss()
        )
