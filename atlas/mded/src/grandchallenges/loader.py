...
class GrandChallengesLoader:
    ...
    def __init__(
        self, loader_settings, prediction_json_filepath="/input/predictions.json"
    ):
        """
    ...
    def mapping(self):
        ...
        for entity in self.json_data:
            ...
            output_path_list = (
                []
            )  # list of the expected output path based on the input filename
            # Get input list
            for channel_data in ent_inputs:
                if (
                    channel_data["interface"]["slug"]
                    in self.loader_settings["InputSlugs"]
                ):
                    ...
                    out_path = os.path.join(
                        self.loader_settings["GroundTruthRoot"], inp_filepath
                    )
            ...
            for channel_data in ent_outputs:
                if (
                    channel_data["interface"]["slug"]
                    in self.loader_settings["OutputSlugs"]
                ):
    ...
    @staticmethod
    def write_image_like(
        data_to_write: np.array, image_to_imitate: str, output_root: str
    ):
        """
    ...
    def write_images_like(
        self, data_to_write: np.array, images_to_imitate: list, output_root: str
    ):
        """
        ...
        if data_to_write.shape[0] != len(images_to_imitate):
            raise ValueError(
                f"data batch size ({data_to_write.shape[0]}) doesn"
                f"t match images_to_imitate "
                f"{len(images_to_imitate)}"
            )

        for i in range(len(images_to_imitate)):
            self.write_image_like(
                data_to_write=data_to_write[i, 0, ...],
    ...
    def load_eval_batches(self):
        ...
        for idx in range(0, len(pred_list), self.batch_size):
            ...
            batch_truth_list = [
                self.prediction_mapping[os.path.basename(pred)]
